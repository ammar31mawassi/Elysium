    ...
    <PageLayout wide>
      ...
      <div className="grid grid-cols-[minmax(0,1fr)] gap-5 lg:grid-cols-[minmax(0,1.6fr)_minmax(300px,.85fr)]">
        <div className="min-w-0 space-y-7">
          ...
          <section><SectionTitle title={p("home_for_you")} action={p("see_all")} to="/discover" /><div className="grid gap-3 sm:grid-cols-2"><RecommendationCard icon={Users} tone="social" eyebrow={p("home_social")} title={model.event?.title || p("discover_social")} detail={model.event ? [format(model.event.parsedDate, "EEE, MMM d · HH:mm"), model.event.location].filter(Boolean).join(" · ") : p("discover_body")} to="/discover?tab=social" /><RecommendationCard icon={BookOpenCheck} tone="study" eyebrow={p("home_study")} title={model.session?.title || p("discover_groups")} detail={model.session?.course_name || model.session?.location || "Add active courses in Me"} to="/discover?tab=sessions" /></div></section>
        </div>
...
function RecommendationCard({ icon: Icon, tone, eyebrow, title, detail, to }) { return <Link to={to} className={cn("rounded-lg border border-border bg-card p-4", domainTones[tone].border)}><div className="flex items-start gap-3"><span className={cn("flex h-10 w-10 shrink-0 items-center justify-center rounded-md", domainTones[tone].icon)}><Icon className="h-5 w-5" /></span><div className="min-w-0"><p className={cn("text-xs font-semibold", domainTones[tone].text)}>{eyebrow}</p><h3 className="mt-1 line-clamp-1 text-sm font-bold text-foreground">{title}</h3><p className="mt-1 line-clamp-2 text-xs leading-relaxed text-muted-foreground">{detail}</p></div></div></Link>; }
