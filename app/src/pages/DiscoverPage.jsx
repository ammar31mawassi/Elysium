...
export default function DiscoverPage() {
  ...
  return <PageLayout wide>
    ...
    {!loading && activeItems.length === 0 && <EmptyState icon={Search} title={p("discover_empty")} message={tab === "social" ? "No upcoming campus activities match this search." : tab === "sessions" ? "Add active courses in Me to see relevant study groups." : p("discover_body")} />}
    {selected && <DetailsModal selected={selected} user={user} profile={profile} onClose={() => setSelected(null)} />}
  </PageLayout>;
}

function Card({ children, tone }) { return <article className={cn("flex min-h-60 min-w-0 flex-col rounded-lg border border-border bg-card p-4", tone ? domainTones[tone].border : "")}>{children}</article>; }
...
function DetailsModal({ selected, user, profile, onClose }) {
  ...
  return <Modal title={item.title || "Study group"} onClose={onClose}><div className="space-y-4" dir="auto"><p className={cn("text-xs font-semibold", domainTones[social ? "social" : "study"].text)}>{social ? item.activity_name || item.category : `${item.is_marathon ? "Study marathon" : "Study group"} · ${item.course_name}`}{item.preferred_language ? ` · ${item.preferred_language}` : ""}</p>{(item.description || item.notes) && <p className="text-sm leading-relaxed text-muted-foreground">{item.description || item.notes}</p>}<div className="rounded-md border border-border p-3"><p className="text-xs font-semibold text-muted-foreground">{social ? "Created by" : "Hosted by"}</p><p className="mt-1 text-sm font-semibold text-foreground">{creatorName}</p>{creatorDetails.some(Boolean) && <p className="mt-1 text-xs text-muted-foreground">{creatorDetails.filter(Boolean).join(" · ")}</p>}</div><div className="rounded-md bg-muted/50 p-3 text-sm text-foreground"><p>{social ? `${item.date}${item.start_time ? `, ${item.start_time}` : ""}` : new Date(item.session_date).toLocaleString()}</p>{item.location && <p className="mt-1 text-muted-foreground">{item.location}</p>}</div></div></Modal>;
}
