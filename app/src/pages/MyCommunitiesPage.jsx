...
function CommunityCard({ type, title, label, status, canceled, date, location, language, participants, capacity, onOpen }) {
  const tone = type === "study" ? "study" : "social";
  return (
    <button onClick={onOpen} className={cn("w-full rounded-lg border border-border bg-background p-4 text-start transition-colors", domainTones[tone].border)}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className={cn("text-xs font-semibold", domainTones[tone].text)}>{label}</p>
          <h3 className="mt-1 truncate font-semibold text-foreground">{title || "Untitled"}</h3>
        </div>
        <span className={cn("shrink-0 rounded-full px-2.5 py-1 text-xs font-bold", canceled ? "bg-destructive/10 text-destructive" : domainTones[tone].badge)}>{status}</span>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between gap-3 border-t border-border pt-3">
        <span className="flex items-center gap-1.5 text-xs font-semibold text-foreground"><Users className="h-3.5 w-3.5" />{participants} / {capacity || "-"} joined</span>
        <span className={cn("text-xs font-bold", domainTones[tone].text)}>View joiners</span>
      </div>
    </button>
  );
}
